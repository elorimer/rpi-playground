 ...
 */
void execute_sha256_cpu(uint32_t * data, uint32_t * H, int stride, int nlaps)
{
    uint32_t W[64];
 ...
 * data is an array of BUFFER_SIZE buffers to hash
 * H is an input/output parameter
 * stride is the stride for data
 */
void execute_sha256_qpu(uint32_t *data, uint32_t *H, int stride, int nlaps)
{
    SHA256ExecuteQPU(H, nlaps);
}
...
int main(int argc, char **argv)
{
    bool run_cpu(true);
    if (argc < 3) {
        fprintf(stderr, "Usage: %s <input file> <nlaps> [-qpu]\n", argv[0]);
        return 1;
    }
    ...
    int nlaps = atoi(argv[2]);
    printf("Running %d laps ...\n", nlaps);
     ...
     */
    printf("Executing %s version ...\n", run_cpu ? "CPU" : "QPU");
    for (int i=0; i < nblocks; i++)
    {
        if (run_cpu)
            execute_sha256_cpu(buffer+i*16, H, stride, nlaps);
        else
            execute_sha256_qpu(buffer+i*16, H, stride, nlaps);
    }
    ...
    // print out the H
    if (!run_cpu) SHA256FetchResult(H);
